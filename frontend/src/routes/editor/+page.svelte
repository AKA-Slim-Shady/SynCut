<script lang="ts">
  ...
  import {
    Upload, Play, Pause, SkipBack, SkipForward, Scissors, Copy, Trash2, Plus, Film, MonitorPlay, Image as ImageIcon, Music, Settings, Download
  } from '@lucide/svelte';
  ...
  let isUploading = $state(false); // To show loading state on the button
  ...
  function requestDriveAccess() {
    if (typeof google !== 'undefined' && google.accounts?.oauth2) {
      ...
      console.error("Google APIs failed to load. Check your index.html scripts.");
    }
  }
  ...
  async function pickerCallback(data: any) {
    if (data.action === google.picker.Action.PICKED) {
      ...
      try {
        // Send the file details to your Bun Backend
        ...
        if (response.ok) {
           // Success! Add the new file to the Sidebar UI
           uploadedAssets = [...uploadedAssets, {
               name: file.name,
               // Basic logic to determine icon type based on filename extension
